...
function runCommand(binary, args) {
  return new Promise((resolvePromise, rejectPromise) => {
    const commandProcess = spawn(binary, args, {
      ...
      env: { ...process.env, DECENTRA_E2E_SIGNUP_EMAIL: '1' }
    })
...
/**
 * E2E with Synapse configured for 3pid email (MailHog), then Cucumber
 * scenarios tagged @email_signup. Requires Docker.
 */
async function main() {
  ...
  try {
    await runCommand('npm', ['run', 'test:e2e:run:email'])
  } catch (error) {
    testFailed = true
    console.error(
