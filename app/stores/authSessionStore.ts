...
async function encryptAttachmentData(data: ArrayBuffer): Promise<{
  ...
  for (let index = 8; index < ivBytes.length; index += 1) {
    ivBytes[index] = 0;
  }
...
function isRoomEncrypted(room: sdk.Room): boolean {
  const hasEncryptionStateEvent = (
    room as sdk.Room & { hasEncryptionStateEvent?: () => boolean }
  ).hasEncryptionStateEvent;
  if (typeof hasEncryptionStateEvent === "function") {
    return hasEncryptionStateEvent.call(room);
  }
  const encryptionStateEvent = room.currentState?.getStateEvents?.(
    "m.room.encryption",
    "",
  );
...
export const useAuthSessionStore = defineStore("authSession", () => {
  ...
  const sessionRestoreStatus = ref<SessionRestoreStatus>("idle");
  const isLoggedIn = computed(() => client.value !== null);
  const userId = computed(() => {
    const matrixClient = client.value as
      | (MatrixClient & { getUserId?: () => string | null })
      | null;
